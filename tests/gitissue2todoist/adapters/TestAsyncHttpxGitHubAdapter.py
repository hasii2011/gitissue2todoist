...
class TestAsyncHttpxGitHubAdapter(UnitTestBase, IsolatedAsyncioTestCase):
    """
    ...
    async def testGetRepositoryNames(self):
        if self._gitHubToken == '':
            self.logger.warning(f'Cannot run test either token or user name not set in environment')
        else:
            adapter: AsyncHttpxGitHubAdapter = AsyncHttpxGitHubAdapter(authenticationToken=self._gitHubToken)

            slugs: Slugs = await adapter.getRepositoryNames()
            self.assertTrue(len(slugs) != 0, 'We should find someting')

    async def testGetMileStoneTitles(self):
        ...
        if self._gitHubToken == '':
            ...
            mileStoneTitles: MilestoneTitles = await adapter.getMileStoneTitles(repoName=TEST_REPO_NAME)

            self.assertTrue(len(mileStoneTitles) != 0, 'We should find some milestones')

    async def testGetAbbreviatedIssues(self):
        if self._gitHubToken == '':
            self.logger.warning(f'Cannot run test either token or user name not set in environment')
        else:
            adapter: AsyncHttpxGitHubAdapter = AsyncHttpxGitHubAdapter(authenticationToken=self._gitHubToken)

            abbreviatedGitIssues: AbbreviatedGitIssues = await adapter.getAbbreviatedIssues(repoName=TEST_REPO_NAME, milestoneTitle=TEST_MILESTONE_TITLE)
    ...
    async def testGetIssuesAssignedToOwner(self):
        if self._gitHubToken == '':
            self.logger.warning(f'Cannot run test either token or user name not set in environment')
        else:
            adapter: AsyncHttpxGitHubAdapter = AsyncHttpxGitHubAdapter(authenticationToken=self._gitHubToken)
            abbreviatedGitIssues: AbbreviatedGitIssues = await adapter.getIssuesAssignedToOwner(
                slugs=TEST_SLUGS,
...
def suite() -> TestSuite:
    ...
    testSuite.addTest(unittest.defaultTestLoader.loadTestsFromTestCase(testCaseClass=TestAsyncHttpxGitHubAdapter))
